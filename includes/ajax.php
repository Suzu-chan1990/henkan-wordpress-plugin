...
function henkan_convert() {
    ...
    if ( is_numeric( $item ) ) {
        ...
        if ( $file ) {
            ...
            $msg = $res ? sprintf( __( 'ID %d optimiert', 'henkan-webp-avif-converter' ), $id ) : sprintf( __( 'ID %d geprueft', 'henkan-webp-avif-converter' ), $id );
            wp_send_json_success( [ 'msg' => $msg ] );
        } else {
            /* translators: %d: Attachment ID */
            wp_send_json_error( [ 'msg' => sprintf( __( 'Datei fehlt ID %d', 'henkan-webp-avif-converter' ), $id ) ] );
        }
    } else {
        ...
        if ( file_exists( $path ) ) {
            ...
            $msg = $res ? sprintf( __( '%s optimiert', 'henkan-webp-avif-converter' ), basename( $path ) ) : sprintf( __( '%s geprueft', 'henkan-webp-avif-converter' ), basename( $path ) );
            wp_send_json_success( [ 'msg' => $msg ] );
        } else {
            /* translators: %s: File name */
            wp_send_json_error( [ 'msg' => sprintf( __( 'Nicht gefunden: %s', 'henkan-webp-avif-converter' ), basename( $path ) ) ] );
        }
    }
}
