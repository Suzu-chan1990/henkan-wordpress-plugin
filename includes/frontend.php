...
function henkan_img_callback($matches) {
    ...
    // Priority: AVIF > WebP > JXL (Experimental)
    if (!empty($settings['enable_avif'])) {
        $sources .= '<source srcset="' . $base_url . '.avif" type="image/avif">';
    }
    if (!empty($settings['enable_webp'])) {
        $sources .= '<source srcset="' . $base_url . '.webp" type="image/webp">';
    }
    if (!empty($settings['enable_jxl'])) {
        $sources .= '<source srcset="' . $base_url . '.jxl" type="image/jxl">';
    }
